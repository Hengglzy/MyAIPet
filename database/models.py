...
def get_all_sessions() -> list[dict]:
    with Session(engine) as session:
        statement = select(SessionMeta).order_by(SessionMeta.last_updated.desc())
        results = session.exec(statement).all()
        return [{"session_id": r.session_id, "title": r.title, "summary": r.summary, "time": r.last_updated.strftime("%m-%d %H:%M")} for r in results]
...
def get_recent_context(session_id: str, limit: int = 10):
    with Session(engine) as session:
        statement = select(Message).where(Message.session_id == session_id).order_by(Message.id.desc()).limit(limit)
        results = session.exec(statement).all()
